...
LRESULT CALLBACK MainWindowProcedure( HWND hWndMain, UINT uMessage, WPARAM wParam, LPARAM lParam )
{
	...
	switch( uMessage )
	{
		...
		case WM_SIZE:
		{
			...
			nFileListViewWindowWidth	= ( ( nClientWidth - TREE_VIEW_WINDOW_WIDTH ) + WINDOW_BORDER_WIDTH );

			// Calculate window positions
			nFileListViewWindowLeft		= ( TREE_VIEW_WINDOW_WIDTH - WINDOW_BORDER_WIDTH );

			// Move control windows
			g_folderTreeViewWindow.Move( 0, 0, TREE_VIEW_WINDOW_WIDTH, nTreeViewWindowHeight, TRUE );
			g_fileListViewWindow.Move( nFileListViewWindowLeft, 0, nFileListViewWindowWidth, nTreeViewWindowHeight, TRUE );
...
int WINAPI WinMain( HINSTANCE hInstance, HINSTANCE, LPSTR, int nCmdShow )
{
	...
	if( mainWindowClass.Register() )
	{
		...
		if( mainWindow.Create( MAIN_WINDOW_CLASS_NAME, NULL, hInstance, MAIN_WINDOW_TEXT ) )
		{
			...
			// Auto-size all folder list view window columns
